...
| [recognizedTempResult](#recognizedTempResult) | 临时识别结果 |
| [translatedTempResult](#translatedTempResult) | 临时翻译结果 |
| [startAudioTranslation_pre](#startAudioTranslation_pre)        | 开始前置翻译错误 |
| [startAudioTranslation_post](#startAudioTranslation_post)        | 开始后置翻译错误 |
----------------------------------------
...
| `appSecret`    | app加密秘钥 |
| `asrResult`    | 是否需要识别结果 |
| `transResult`  | 是否需要识别结果 |
| `tempResult`   | 是否需要识别和翻译的临时结果 |
| `userId`       | 用户id(便于查询翻译记录) |
| `srcLanguage`  | 源语言，参考 [支持语言列表](https://docs.ilivedata.com/ts/rtvt/language/) |
| `destLanguage` | 目标语言，参考[支持语言列表](https://docs.ilivedata.com/ts/rtvt/
language/)  |
| `ExtensionInfo`       | 远端用户的信息结构 |
|| `remoteUid`   (远端音频用户ID)|
|| `info.channelId`   (频道ID)  |
