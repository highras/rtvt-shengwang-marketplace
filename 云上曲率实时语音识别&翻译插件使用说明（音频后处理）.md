...
注意：本文内容主要针对使用[声网视频 SDK v4.0.0 Beta](https://docs.agora.io/cn/video-call-4.x-beta/product_video_ng?platform=Android) 并需要进行**音频后处理**操作使用。如果不需要在RTC音频传输过程中，接收端对音频进行识别和翻译，请忽略本文。
...
### 2. 使用插件
在使用插件时，除了在音频前处理的使用方法中一些必填参数以外，还需要由业务侧自行在`AgoraExtensionInfo`方法中填入`remoteUid`（远端用户ID，即音频发送端ID）、`channelId`（音频通话频道ID）和`localUid`（本端用户ID，即音频接收端ID）。
...
```java
    engine.setExtensionProperty(EXTENSION_VENDOR_NAME, EXTENSION_AUDIO_FILTER_VOLUME, extensionInfo, "startAudioTranslation", jsonObject.toString());
```
